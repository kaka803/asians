...
useLayoutEffect(() => {
  ...
  useEffect(() => {
    ...
    const config = ROBOT_CONFIGS[activeSection] || ROBOT_CONFIGS.microsoft;

    // Smooth movement to EXACT previous positions
    gsap.to(robotRef.current, {
      top: config.top,
      x: config.x,
      scale: config.scale,
      rotation: 0, // Reset rotation to original
      duration: 1,
      ease: "power3.inOut",
    });

    // Subtle idle floating animation (vertical only to preserve position)
    const floatingTween = gsap.to(robotRef.current, {
      y: "+=12",
      duration: 2,
      repeat: -1,
      yoyo: true,
